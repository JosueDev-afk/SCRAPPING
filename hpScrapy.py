...
def leerArchivoCSV(file):
    with open(file, 'r') as archivo_csv:
        lectorCSV = csv.reader(archivo_csv,delimiter=',')
        listaTemp=list(lectorCSV)
        for i in range(0,len(listaTemp)):
            skuArray.append(listaTemp[i][0])
            nameProductsArray.append(listaTemp[i][1])
...
def iniciarCodigo(isFile, product=None, file=None):
    resetearArreglosP()
    if (isFile==True):
        leerArchivoCSV(file)
        c=0
        for i in nameProductsArray:
            resetearArreglos()
            obtenerLinksPP(generarLink(i))
            generarPS(i)
            agregarInfoDiccionario(skuArray[c],preciosPPArray,tiendasPPArray,linksPPArray,arregloDicPS)
            c+=1
    else:
        print(False)
        #obtenerLinksPP(generarLink(product))
        #generarPS(product)
    driver.quit()
